...
if (addActionsBtn) {
  addActionsBtn.addEventListener("click", function () {
    const newActions = actionSection.cloneNode(true); //on each click it creates a copy of the input field each time by cloning the first section above with the input field inside // takes off the event handlers
    const input = newActions.getElementsByTagName("input")[0]; // take the first input field in the list out of the section 'newEmotions' and clear it out, we couldn't do this if we didn't grab just one, could have added a class or id to the input and done it that way
    // console.log(input.value)
    input.value = ""; //clear out the value just in case there is already a value
    actionList.appendChild(newActions); //  append new feeling section and input box on click - append the cloned field
  });
}
...
function editEmotionsInfo() {
  const fields = emotionsVariables(); // could I have just grabbed the parent id and then the children so done a query selector on parent instead of select  all and then select all on children instead
...
async function updateEmotionsInfo() {
  ...
  try {
    await fetch("/emotion/:id", {
      ...
      body: JSON.stringify({
        id: id, // could have put this into a data- attribute
